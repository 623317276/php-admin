...
class SylistbaseController extends HomeBaseController
{
    ...
    public function getList(){
        ...
        $token = $result['token'];
	   	...
	   	$model = Db::name('sylistbase')->alias('as a')->join('user b','a.uid=b.id','left')->join('adminuser c','a.cashier=c.id','left')->field('a.*,b.name,c.user_login,c.user_nickname')->where($where);
	   	
		$listinfo = $model->limit(''.$startrow.','.$result['pageSize'])->order("id Desc")->select()->toArray();
	   // echo Db::name('sylistbase')->getlastsql();die;
	   	$total = Db::name('sylistbase')->alias('as a')->join('user b','a.uid=b.id','left')->join('adminuser c','a.cashier=c.id','left')->field('a.*,b.name,c.user_login,c.user_nickname')->where($where)->count();
