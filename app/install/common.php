...
// 发送邮件
function sendMail($subject = 'STD', $body = '', $email = ''){
        if(empty($subject) || empty($body) || empty($email)){
            return json_encode(['code' => -1, lang('MissPara')]);
        }
        // echo 'https://mail.stdchain.app/Rylkzmerwsejrp3n/Reg/card?user_name='.$email.'&body='.urlencode($body);die;
        // 此处用跳板发
        // $result = file_get_contents('https://mail.stdchain.app/Rylkzmerwsejrp3n/Reg/card?user_name='.$email.'&body='.urlencode($body));
        $result = geturldata('https://mail.stdchain.app/Rylkzmerwsejrp3n/Reg/card?user_name='.$email.'&body='.urlencode($body));
        $result = json_decode($result, true);
        return json_encode($result);
        
        //以下也能用，只是邮箱发送成功后可能会到垃圾箱
        // $username = 'std9999@126.com';
        // $password = 'IGJMMNCOGVEPRKBO';
        // $mail = new \PHPMailer(); 
        ...
        // $mail->SMTPSecure = "ssl";// 使用ssl协议方式</span> 
        // $mail->Port = 994;// 163邮箱的ssl协议方式端口号是465/994  
        // $mail->setFrom($username,"STD");// 设置发件人信息，如邮件格式说明中的发件人，这里会显示为Mailer(xxxx@163.com），Mailer是当做名字显示  
        // $mail->addAddress($email,'STD');// 设置收件人信息，如邮件格式说明中的收件人，这里会显示为Liang(yyyy@163.com)  
        // $mail->addReplyTo($username,"Reply");// 设置回复人信息，指的是收件人收到邮件后，如果要回复，回复邮件将发送到的邮箱地址  
        // //$mail->addCC("xxx@163.com");// 设置邮件抄送人，可以只写地址，上述的设置也可以只写地址(这个人也能收到邮件)  
        // //$mail->addBCC("xxx@163.com");// 设置秘密抄送人(这个人也能收到邮件)  
        // //$mail->addAttachment("bug0.jpg");// 添加附件  
        // $mail->Subject = $subject; // "这是一个测试邮件";// 邮件标题  
        // $mail->Body = $body; // "邮件内容是 <b>您的验证码是：123456</b>，哈哈哈！";// 邮件正文  
        // //$mail->AltBody = "This is the plain text纯文本";// 这个是设置纯文本方式显示的正文内容，如果不支持Html方式，就会用到这个，基本无用  
        // if(!$mail->send()){// 发送邮件  
        //     // echo "Message could not be sent.";  
        //     // echo "Mailer Error: ".$mail->ErrorInfo;// 输出错误信息
        //     return json_encode(['code' => -1, $mail->ErrorInfo]);
        // }else{  
        //     // echo '发送成功';  
...
function get_ticket(){
    //STD行情
	@$stdinfo = file_get_contents('https://www.pickcoin.top/api/spot/v3/instruments/STD-USDT/ticker');
	@$stdinfo = json_decode($stdinfo,true);
    $arrb = isset($stdinfo['last']) ? $stdinfo['last'] : 2;
    return $arrb * 7.0255;
// return 64.5;
}
