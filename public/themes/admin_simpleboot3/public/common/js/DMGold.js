	...
	$.extend(DMGold,{
		...
		PageTag:{
			...
			init:function(options){
				...
				if(this.pageCount<=this.showPages+2){
					...
					var huy = Math.ceil((this.curPage*1)+ offset);
					var cuy = Math.ceil((this.curPage*1));
					
		            for (var i = this.curPage - offset; i <= huy; ++i) {
		                if (i == this.curPage)
		                {
		                	pageTagDiv.append("<font class=\"cpagebox\">" + i + "</font>");
		                }
				...
				if(this.curPage!=this.pageCount && this.pageCount != 0){
					pageTagDiv.append("<a href=\"Javascript:DMGold.PageTag.toPage(" + ((this.curPage*1) + 1) + ")\" class=\"pagebox\" >" + "下一页 " + "</a>");
				}
		...
		PageTags:{
			...
			init:function(options){
				...
				if(this.curPage!=this.pageCount && this.pageCount != 0){
					pageTagDiv.append("<a href=\"Javascript:DMGold.PageTags.toPage(" + (this.curPage  + 2 - 1) +",'"+pageId+ "')\" class=\"pagebox\" >" + "下一页 " + "</a>");
				}
