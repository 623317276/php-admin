...
function changeCaptcha(id, url) {
   document.getElementById(id).src = "common/captcha.do?ran=" + Math.random();
}
...
var dm={
		...
		*/
		...
		alert:function(msg,option){
			...
			"<div class=\"dialog\" style=\"margin: -230px 0 0 -365px\" id=\"duhua\">"+
			  "<div class=\"clearfix\" >"+
			      "<div class=\"title\">";
		...
		confirm:function(msg,option){
			...
			if(typeof(option)!="undefined"){
				...
				if(typeof(option.showClose)!="undefined"){
					dm.alertShowClose=option.showClose;
				}else{
					dm.alertShowClose = true;
				}
				if(typeof(option.showCancel)!="undefined"){
					dm.alertShowCancel=option.showCancel;
				}else{
					dm.alertShowCancel = true;
				}
			}

			var dh="<div class=\"popup_bg\" id=\"bg2\"></div>"+
			"<div class=\"dialog\" style=\"margin: -230px 0 0 -365px\" id=\"duhua2\">"+
			  "<div class=\"clearfix\" >"+
			      "<div class=\"title\">" ;
			      if(dm.alertShowClose){
			    	  dh =dh+ "<a href=\"#\" onClick=\"dm.closeConfirm()\" class=\"out\"></a>";
			      }
			      dh = dh +dm.confirmTitle+"</div>"+
			      "<div class=\"cotent clearfix\">"+         
			         "<div class=\"conn\"><i class='"+dm.alertPicClass+"'></i>"+msg+"</div>"+
			         "<div class=\"btn\"><a href=\"#\" class=\"btn_cs\" onClick=\"dm.confirmOk()\" >"+dm.confirmOkButton+"</a>";
			         if(dm.alertShowCancel){
				    	  dh =dh+ "<a href=\"#\" onClick=\"dm.confirmCancle()\"  class=\"btn_cs ml20\">"+dm.confirmCancleButton+"</a>";
				      }
			         dh =dh+ "</div>"+
			      "</div>"+     
			    "</div>"+
...
function xgmmSubmit(){
	...
	if(!checkPwd(newpassword)){
		$("#modifypwdmsgTD").html("密码须为6~20个字符，区分大小写！").show();
		return false;
	}
	...
	$.ajax( { 
      type : "POST", 
      url : "/index.php/Manage_Yingxiao/upmangpwd",    
      	...
      	if(result.code == "000000"){
      		...
      		dm.alert("修改成功！",{
      			...
      			callback:function(){	//按确定后执行的函数
      	      	    ...
      	      	    window.location.href='/index.php/Manage_Login/logout';   
      			}
      		}); 
      		setTimeout('window.location.reload();', 2000);  
    	}else{
			...
			if(obj){
				...
				$("#modifypwdmsgTD").html(result.msg).show();
			}
    	} 
      } 
	});
...
function setMenu(topNo,leftNo,leftSubNo,urlLeft,urlMain){
	  ...
	  sessionStorage.setItem("urlMain", urlMain);
	  sessionStorage.setItem("leftNo", leftNo);
	  sessionStorage.setItem("leftSubNo", leftSubNo);
